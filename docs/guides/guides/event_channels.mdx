...
Event channels are regular Discord text channels that Martin will take over,
...
To create an event channel, Martin must have the follow permissions.
...
Users must have the `Manage Server` permission to use the `/sort` command
