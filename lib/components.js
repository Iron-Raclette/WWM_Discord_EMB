...
const specIcons = {
  ...
  'Stonesplit Strength': `<:StonesplitMight:${emojiIds.StonesplitStrength}>`,
  ...
  'Bamboocut Burst': `<:BamboocutWind:${emojiIds.BamboocutBurst}>`,
...
function templateButtons(nonce, selectedTimezone = 'Europe/Paris') {
  return [
    ...
    {
      type: 1,
      components: Object.entries(templates).filter(([, tpl]) => tpl.botAccessible !== false).map(([key, tpl]) => ({ type: 2, style: 1, custom_id: `template:${nonce}:${key}`, label: tpl.title }))
    },
    {
      type: 1,
