const { templates } = require('./racletteEvents');

const emojiIds = {
  Tanklogo: '1443988426432057424',
  DPS: '1443988725410566315',
  Healer: '1443988649585938495',
  BellstrikeSplendor: '1517645399781937264',
  BellstrikeUmbra: '1517645414046761071',
  BamboocutWind: '1517645384401420540',
  BamboocutBurst: '1517645369134157956',
  SilkbindJade: '1517645476751741081',
  SilkbindDeluge: '1517645457646551070',
  StonesplitMight: '1517645497811075244',
  StonesplitStrength: '1517645515104321557',
  Hybrid: '1517645438918987776'
};
...
function templateButtons(nonce, selectedTimezone = 'Europe/Paris') {
  const visibleTemplates = Object.entries(templates).filter(([, tpl]) => tpl.botAccessible !== false);
  
  const templateRows = [];
  for (let i = 0; i < visibleTemplates.length; i += 5) {
    const chunk = visibleTemplates.slice(i, i + 5);
    templateRows.push({
      type: 1,
      components: chunk.map(([key, tpl]) => ({
        type: 2, style: 1, custom_id: `template:${nonce}:${key}`, label: tpl.title
      }))
    });
  }
  ...
  return [
    ...
    {
      type: 1,
      components: [{ type: 2, style: 2, custom_id: `upload:hint:${nonce}`, label: '📤 Upload local' }]
    }
  ];
}
...
function classButtons(eventId, role) {
  const byRole = {
    dps: ['Bellstrike Splendor', 'Bellstrike Umbra', 'Bamboocut Wind', 'Bamboocut Burst', 'Silkbind Jade', 'Stonesplit Strength', 'Hybrid'],
    tank: ['Stonesplit Might', 'Hybrid'],
    healer: ['Silkbind Deluge', 'Hybrid']
  };
